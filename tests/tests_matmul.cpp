...
namespace tBLAS_test
{
    ...
    TEST_CASE("Matmul INT Square", "[matmul]")
    {
        ...
        SECTION("XL kernel")
        {
            compare_INT_2D(tBLAS_test::Trivial::matmul(A, B), tBLAS::multiply(tA, tB, tBLAS::BLAS::matmul_kernel::xl));
        }
        SECTION("SM kernel")
        {
            compare_INT_2D(tBLAS_test::Trivial::matmul(A, B), tBLAS::multiply(tA, tB, tBLAS::BLAS::matmul_kernel::sm));
        }
    }
    ...
    TEMPLATE_TEST_CASE("Matmul FP Square", "[matmul]", float, double)
    {
        ...
        SECTION("XL kernel")
        {
            compare_FP_2D(tBLAS_test::Trivial::matmul(A, B), tBLAS::multiply(tA, tB, tBLAS::BLAS::matmul_kernel::xl));
        }
        SECTION("SM kernel")
        {
            compare_FP_2D(tBLAS_test::Trivial::matmul(A, B), tBLAS::multiply(tA, tB, tBLAS::BLAS::matmul_kernel::sm));
        }
    }
